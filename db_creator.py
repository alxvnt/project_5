...
def csv_reader(file2, conn):

    cursor = conn.cursor()
    csv.field_size_limit(sys.maxsize)
    file =  open(file2, newline='', encoding ="utf8")
    reader = csv.reader(file, delimiter = '	')
    header = next(reader)
    # Get all the data we need from each line of the csv
    for row in reader:
        try:
            list1 = []
            list1.append([str(row[7]), categ_pars(row[16]),
                        str(row[12]), str(row[30]), str(row[164]), str(row[1])])
            # If the data are not too long
            if ((0 < len(list1[0][0]) < 30) and (list1[0][1]) and
                (0 < len(list1[0][2]) < 25) and (0 < len(list1[0][3]) < 25) and
                (list1[0][4]) and (0 < len(list1[0][5]) < 100)):
                print(list1)
                ...
                # Insert data in the table product
                cursor.execute("""
                    INSERT INTO product
                    SET name = %s,
                    id_category = (SELECT id_category
...
# Test if a csv categories contains a categories of the list
def categ_pars(var):
...
# Create and return an acces to the database
def db_connect():
...
def create_db(cursor):
    ...
    cursor.execute(""" CREATE TABLE saved_substitute (
                        id_saved_substitute INT UNSIGNED
